...
def main():
    ...
    logging.info('Starting ddGhydr_train_{}.py.'.format(model_type))
    ...
    save_csv(dtrain_df, 'dtrain_data')
    dtest_df = calc_relative_dataframe(test_df)
    save_csv(dtest_df, 'dtest_data')
    ...
    kfolds = split_dataset(dtrain_df, n_splits, random_state)
    run_regressor(kfolds)
    print('Training complete.')
    logging.info('Training complete.')

    logging.info('Finished ddGhydr_train_{}.py.'.format(model_type))
...
def run_regressor(kfolds):
    ...
    for model in all_models:
        ...
        with open(output_dr + 'fold_' + str(internal_fold_num) + '_internal_validation.csv', 'w') as file:
            ...
            writer.writerow(['ID', 'Experimental ddGoffset (kcal/mol)', 'Predicted ddGoffset (kcal/mol)'])
    ...
    # Save to CSV
    cumulative_MAEtauR_df_save_loc = output_dr + 'ddGoffset_SVR_MAEtauR_outputs.csv'
    ...
    # Save to CSV
    cumulative_MAE_save_loc = output_dr + 'ddGoffset_' + model_type +  '_BO_MAE.csv'
...
def calc_relative_dataframe(dataframe):
    ...
    dID = [col + '~' + row for col in ID for row in ID]

    # compute relative labels
    y = dataframe.pop('ddGoffset (kcal/mol)').tolist()
...
def get_labels():
    ...
    exp_offset_with_errors_df = pd.DataFrame(offsets, columns=['ID', 'ddGoffset (kcal/mol)', 'uncertainty (kcal/mol)'])
...
def convert_to_float(dataframe):
    ...
    float_df = float_df.rename(columns={'dGhydr (kcal/mol)': 'ddGoffset (kcal/mol)'})
