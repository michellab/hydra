...
def main():
    ...
    true_y = pd.read_csv(datasets_dr + 'labels_y/null_experimental_labels.csv', index_col='ID').sort_index()
...
def create_relative_train_test(dataframe):
    ...
    for set in set_type:
        # setup HDF5 file
        hdf = pd.HDFStore(datasets_dr + 'null_d' + set + '_data.h5')
...
def create_absolute_train_test(features, labels):
    ...
    for set in set_type:
        # setup HDF5 file
        hdf = pd.HDFStore(datasets_dr + 'null_' + set + '_data.h5')
