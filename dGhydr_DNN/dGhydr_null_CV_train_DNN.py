...
def main():
    ...
    with open(output_dr + '{}_statistics.csv'.format(model_type), 'w') as file:
...
def train_model(fold, fold_num):
    ...
    def fitness(num_dense_layers_base, num_dense_nodes_base,
        ...
        parameters = [num_dense_layers_base, num_dense_nodes_base,
                      num_dense_layers_end, num_dense_nodes_end,
                      adam_b1, adam_b2, adam_eps]
        with open(output_dr + '{}_statistics.csv'.format(model_type), 'a') as file:
            writer = csv.writer(file)
        ...
        if mae < best_mae:
            ...
            model.save(output_dr + 'dGhydr_' + model_type + '_fold_' + str(fold_num) + '_model.h5')
...
def split_dataset(dataset, n_splits, random_state):
    ...
    with open(output_dr + 'kfolds.json', "wb") as f:
        pickle.dump(kfolds, f)

    print('Pickled kfolds nested list.')
    logging.info('Pickled kfolds nested list at {}kfolds.json.'.format(output_dr))
